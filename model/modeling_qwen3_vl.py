...
from transformers.utils import TransformersKwargs, can_return_tuple, torch_compilable_check
...
class Qwen3VLModelOutputWithPast(ModelOutput):
    ...
    deepstack_features: list[torch.FloatTensor] | None = None  # used by the DPT Depth Head
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # ===== Save a copy of DeepStack features for the DPT Depth Head =====
        saved_deepstack_features = None
        if deepstack_visual_embeds is not None:
            saved_deepstack_features = [feat.clone() for feat in deepstack_visual_embeds]
        ...
        return Qwen3VLModelOutputWithPast(
            **outputs,
            ...
            deepstack_features=saved_deepstack_features,
        )
...
class Qwen3VLCausalLMOutputWithPast(ModelOutput):
    ...
    attentions: tuple[torch.FloatTensor] | None = None
    rope_deltas: torch.LongTensor | None = None
    # ===== DPT Depth Head outputs =====
    depth_loss: torch.FloatTensor | None = None
    """SILog regression loss from the DPT Depth Head."""
    depth_pred: list[torch.FloatTensor] | None = None
    """Per-sample list of pixel-level depth maps, each of shape (H_out, W_out)."""
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        depth_head_type = _os_for_depth.environ.get(
            ...
            getattr(config, "depth_head_type", "dpt"),
        )
        ...
        if depth_head_type == "mlp":
            ...
            feature_mode = _os_for_depth.environ.get(
                "DEPTHLM_DEPTH_HEAD_FEATURE_MODE",
                getattr(config, "depth_head_feature_mode", "llm_last"),
            )
            config.depth_head_mlp_hidden = mlp_hidden
            config.depth_head_feature_mode = feature_mode
            self.depth_head = MLPDepthHead(
    ...
    @staticmethod
    def _silog_loss(pred: torch.Tensor, gt: torch.Tensor, lambda_: float = 0.5) -> torch.Tensor:
        """Scale-Invariant Logarithmic Loss (SILog).

        Standard loss in depth estimation; computes error in log space so near and
        far regions are weighted equally.

        Formula: sqrt( mean(d^2) - lambda * mean(d)^2 )
        where d = log(pred) - log(gt)

        Args:
            pred: predicted depth values (must be > 0).
            gt: GT depth values (must be > 0).
            lambda_: scale-invariance coefficient, standard value 0.5.
        Returns:
            SILog loss value.
        """
        log_diff = torch.log(pred) - torch.log(gt)
        silog = torch.sqrt((log_diff ** 2).mean() - lambda_ * (log_diff.mean() ** 2) + 1e-8)
        return silog
    ...
    def forward(
        ...
        # ===== Depth args =====
        pixel_depth_labels: list[torch.FloatTensor] | None = None,  # dense depth GT for each image
        **kwargs: Unpack[TransformersKwargs],
    ) -> tuple | Qwen3VLCausalLMOutputWithPast:
        """
        Qwen3-VL forward with an added DPT Depth Head branch.

        The DPT Head consumes 4 layers of features:
            - Layer 0-2: ViT DeepStack features (layer 5, 11, 17)
            - Layer 3: image-token positions of the LLM last hidden states

        Outputs a pixel-level depth map (at the original image resolution).
        """
        ...
        run_depth_head = (
            ...
            and past_key_values is None  # not in token-by-token generate decoding
            and (pixel_depth_labels is not None or not self.training)  # train w/ GT, or eval forward
        )

        if run_depth_head:
            image_token_mask = (input_ids == self.config.image_token_id).to(hidden_states.device)
            ...
            if image_token_mask.any():
                ...
                for b in range(hidden_states.shape[0]):
                    ...
                    num_img_tokens = img_mask_b.sum().item()

                    if num_img_tokens == 0:
                        continue

                    # --- Build the 4-layer features ---
                    # DeepStack features are concatenated across the batch; slice by offset
                    offset = 0
                    for prev_b in range(b):
                        offset += (input_ids[prev_b] == self.config.image_token_id).sum().item()

                    ds_feats_b = []
                    for ds_feat in deepstack_features:
                        ds_feats_b.append(ds_feat[offset:offset + num_img_tokens])

                    # Image-token positions of the LLM last hidden states
                    llm_feat_b = hidden_states[b][img_mask_b]

                    # 4 layers: [ds_layer5, ds_layer11, ds_layer17, llm_last]
                    features_4layers = ds_feats_b + [llm_feat_b]

                    # --- Determine patch grid size ---
                    if image_grid_thw is not None:
                        img_idx = self._get_image_index_for_sample(input_ids, b)
                        t, h_vit, w_vit = image_grid_thw[img_idx].tolist()
                        patch_h = h_vit // spatial_merge_size
                        patch_w = w_vit // spatial_merge_size
                    else:
                        patch_h = int(num_img_tokens ** 0.5)
                        patch_w = num_img_tokens // patch_h

                    # --- Compute pixel-level output_size (matches the image resolution) ---
                    # effective_patch_size = vit_patch_size * spatial_merge_size = 16 * 2 = 32
                    effective_patch = self.config.vision_config.patch_size * spatial_merge_size
                    output_h = patch_h * effective_patch  # e.g., 45 * 32 = 1440
                    output_w = patch_w * effective_patch  # e.g., 60 * 32 = 1920
                    output_size = (output_h, output_w)

                    # --- DPT Head forward ---
                    features_batched = [f.unsqueeze(0) for f in features_4layers]
                    depth_map = self.depth_head(features_batched, patch_h, patch_w, output_size=output_size)
                    # depth_map: (1, output_h, output_w)

                    depth_pred_list.append(depth_map.squeeze(0))  # (output_h, output_w)

                    # --- Compute SILog loss ---
                    if pixel_depth_labels is not None and pixel_depth_labels[b] is not None:
                        gt_map = pixel_depth_labels[b].to(depth_map.device, depth_map.dtype)
                        # gt_map: (1, H_gt, W_gt) or (H_gt, W_gt)
                        if gt_map.dim() == 3:
                            gt_map = gt_map.squeeze(0)  # (H_gt, W_gt)

                        pred_map = depth_map.squeeze(0)  # (output_h, output_w)

                        # If resolutions differ, resize pred to gt resolution
                        ...
                        if valid.sum() > 0:
                            depth_losses.append(
                                self._silog_loss(pred_flat[valid], gt_flat[valid])
                            )
        ...
        total_loss = None
        if depth_loss is not None and lm_loss is not None:
            total_loss = lm_loss + depth_loss_weight * depth_loss
        elif depth_loss is not None:
    ...
    def _get_image_index_for_sample(self, input_ids: torch.LongTensor, sample_idx: int) -> int:
        """Return the index of the first image of the sample-th sample in image_grid_thw."""
        vision_start_token_id = self.config.vision_start_token_id
        image_token_id = self.config.image_token_id

        cumulative_images = 0
        for b in range(sample_idx):
            ids = input_ids[b]
            vision_start_indices = (ids == vision_start_token_id).nonzero(as_tuple=True)[0]
            for vs_idx in vision_start_indices:
                if vs_idx + 1 < len(ids) and ids[vs_idx + 1] == image_token_id:
                    cumulative_images += 1
