...
def main():
    ...
    parser.add_argument("--target_split", type=str, default="val",
                        help="Which split to filter from CSV: train/val/test (default: val)")
    ...
    if args.split_file.endswith(".csv"):
        ...
        with open(args.split_file, "r") as f:
            ...
            for row in reader:
                if "split" in row and "scene" in row:
                    if row["split"].strip() == args.target_split:
                        split_scenes.add(row["scene"].strip())
    else:
        with open(args.split_file, "r") as f:
