...
Usage:
    python sample_images_scannet.py \
        --input_jsonl annotations/scannetpp/scannetpp_pixel_depth_val.jsonl \
        --split_file annotations/scannetpp/s3d_val.txt \
        --total_samples 1000 \
        --output_jsonl annotations/scannetpp/test/scannetpp_pixel_depth_val_1000.jsonl \
...
def main():
    ...
    print(f"Split scenes: {len(split_scenes)}")

    # === Pass 1: Fast scan, extract scene_id and line numbers (no depth check) ===
    ...
    num_scenes = len(found_scenes)
    if num_scenes == 0:
        print("ERROR: No matching scenes!")
        return

    # === Compute per-scene allocation (multi-round redistribution) ===
    ...
    # Initial selection: uniform interval per scene
    scene_selected_set = {}
