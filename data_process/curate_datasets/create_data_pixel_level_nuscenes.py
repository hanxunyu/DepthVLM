#!/usr/bin/env python
"""
Create pixel-level depth dataset metadata for NuScenes v2.
...
Usage:
    python create_data_pixel_level_nuscenes.py \
        --data_root /path/to/nuscenes_v2 \
        --splits train,val,test \
        --output_dir ./annotations/nuscenes \
        --num_workers 32
"""
import argparse
import json
...
# NuScenes depth: uint16 PNG, depth_value / DEPTH_SCALE = meters
DEPTH_SCALE = 256.0
...
def process_single_frame(args_tuple):
    ...
    try:
        ...
        return {
            ...
            "camera": camera,
            "split": split,
            "image": f"nuscenes_v2/{split}/rgb/{scene_id}/{rgb_fname}",
            "depth_path": f"nuscenes_v2/{split}/depth/{scene_id}/{depth_fname}",
...
def main():
    parser = argparse.ArgumentParser(description="Create pixel-level depth metadata for NuScenes v2.")
    parser.add_argument("--data_root", type=str, required=True,
                        help="Root dir of nuscenes_v2 (contains train/, val/)")
    ...
    for split in splits:
        ...
        if records:
            output_path = os.path.join(args.output_dir, f"nuscenes_pixel_depth_{split}.jsonl")
