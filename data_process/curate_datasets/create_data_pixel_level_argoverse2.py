#!/usr/bin/env python
"""
Create pixel-level depth dataset metadata for Argoverse v2.
...
# Argoverse v2 depth: uint16 PNG, depth_value / DEPTH_SCALE = meters
DEPTH_SCALE = 256.0
...
def process_single_frame(args_tuple):
    ...
    try:
        ...
        return {
            ...
            "camera": camera,
            "split": split,
            "image": f"argoverse_v2/{split}/rgb/{scene_id}/{rgb_fname}",
            "depth_path": f"argoverse_v2/{split}/depth/{scene_id}/{depth_fname}",
...
def main():
    parser = argparse.ArgumentParser(description="Create pixel-level depth metadata for Argoverse v2.")
    parser.add_argument("--data_root", type=str, required=True,
                        help="Root dir of argoverse_v2 (contains train/, val/, test/)")
    ...
    for split in splits:
        ...
        if records:
            output_path = os.path.join(args.output_dir, f"argoverse_pixel_depth_{split}.jsonl")
