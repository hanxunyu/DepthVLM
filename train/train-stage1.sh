...
echo "[dist] NNODES=$NNODES"
echo "[dist] NPROC_PER_NODE=$NPROC_PER_NODE"
echo "[dist] MASTER_ADDR=$MASTER_ADDR"
echo "[dist] MASTER_PORT=$MASTER_PORT"
echo "[dist] NODE_RANK=$NODE_RANK"
...
############################
# CUDA memory allocator tuning (recommended for 8B model + variable-length inputs)
############################
...
############################
GLOBAL_BATCH_SIZE=2560      # desired global batch size
LOCAL_BATCH_SIZE=32        # batch size per GPU

TOTAL_GPUS=$(( NNODES * NPROC_PER_NODE ))
GLOBAL_STEP_BATCH=$(( TOTAL_GPUS * LOCAL_BATCH_SIZE ))

# Compute gradient accumulation steps (round up)
...
echo "[train] GLOBAL_STEP_BATCH=$GLOBAL_STEP_BATCH"
echo "[train] GRAD_ACC_STEPS=$GRAD_ACC_STEPS (to reach >= $GLOBAL_BATCH_SIZE)"
